...
- Added scanner for `/var/db/diagnostics/signpost`, `special`, and `persist`.
- Added one-tap diagnostic cleanup that removes contents while preserving those three parent directories.
- Cleanup rescans after completion and reports freed space and failed items.
- Existing app-cache cleaner and system-control UI remain intact.

## 1.1.2 (custom build)

- Added System Controls for `com.apple.thermalmonitord` in launchd `disabled.plist`.
- Added OTA enable/disable by managing the standard OTA launchd disable keys.
- Added a narrow global cache/log cleaner for `/var/mobile/Library/Caches` and diagnostic log folders.
- Original `disabled.plist` is backed up once in the app support backups folder before modification.
