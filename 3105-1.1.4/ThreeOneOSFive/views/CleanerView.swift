...
struct CleanerView: View {
    ...
    @State private var diagnosticUsage: [DiagnosticDirectoryUsage] = []
    @State private var isCleaningDiagnostics = false
    ...
    private var isBusy: Bool {
        isScanning || isCleaning || isCleaningDiagnostics
    }

    private var diagnosticBytes: Int64 {
        diagnosticUsage.reduce(0) { $0 + $1.bytes }
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(item: $selectedDiagnosticDirectory) { directory in
                DiagnosticFilesView(
                    directory: directory,
                    sizeText: sizeText
                )
            }
            .sheet(isPresented: $showingTopStorageFiles) {
                StorageTopFilesView(files: topStorageFiles, sizeText: sizeText)
            }
            .onAppear {
    ...
    private var diagnosticSection: some View {
        Section {
            ForEach(diagnosticUsage) { usage in
                Button {
                    selectedDiagnosticDirectory = usage
                } label: {
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .trailing, spacing: 2) {
                            Text(sizeText(usage.bytes))
                                .font(.caption.monospacedDigit())
                                .foregroundStyle(.primary)
                            Text(language.text("cleaner.diagnostics_file_count", Int64(usage.itemCount)))
            ...
            HStack {
                Text(language.text("cleaner.diagnostics_total"))
                    .fontWeight(.semibold)
                Spacer()
                Text(sizeText(diagnosticBytes))
                    .font(.subheadline.monospacedDigit())
            }

            Button {
                activeAlert = .diagnosticConfirmation
            } label: {
                HStack(spacing: 8) {
                    if isCleaningDiagnostics {
                        ProgressView()
                    } else {
                        Image(systemName: "trash")
                    }
                    Text(isCleaningDiagnostics ? language.text("cleaner.diagnostics_cleaning") : language.text("cleaner.diagnostics_clean"))
                        .fontWeight(.semibold)
                }
                .frame(maxWidth: .infinity, minHeight: 44)
            }
            .buttonStyle(.borderedProminent)
            .disabled(diagnosticBytes == 0 || isBusy)
            .padding(.vertical, 4)
        } header: {
            Label(language.text("cleaner.diagnostics"), systemImage: "waveform.path.ecg")
        } footer: {
            Text(language.text("cleaner.diagnostics_footer"))
        }
    }

    private var storageHotspotSection: some View {
        Section {
            ForEach(storageHotspots) { usage in
                Button {
                    topStorageFiles = usage.files
                    showingTopStorageFiles = true
                } label: {
                    HStack(spacing: 10) {
                        Image(systemName: usage.name.lowercased().contains("thumbnail") ? "photo.on.rectangle" : "folder")
                            .foregroundStyle(.secondary)
                            .frame(width: 24)
                        VStack(alignment: .leading, spacing: 2) {
                            Text(usage.name)
            ...
            Button {
                topStorageFiles = DiagnosticCleanupService.top100StorageFiles()
                showingTopStorageFiles = true
            } label: {
                HStack {
                    Image(systemName: "chart.bar.xaxis")
                    Text("Top 100 terbesar dari semua lokasi")
                        .fontWeight(.semibold)
                    Spacer()
                    Image(systemName: "chevron.right")
                        .font(.caption.weight(.semibold))
            ...
            .disabled(isBusy || storageHotspots.isEmpty)
        } header: {
            Label("Media Storage Scanner", systemImage: "internaldrive")
        } footer: {
            Text("Scanner read-only untuk LoFiCloudAssets, PhotoData dan CloudAssets. Tidak ikut dihapus oleh Clean Diagnostics.")
        }
    }
    ...
    private func alert(for alert: CleanerAlert) -> Alert {
        switch alert {
        case .diagnosticConfirmation:
            return Alert(
                title: Text(language.text("cleaner.diagnostics_confirm_title")),
                message: Text(language.text("cleaner.diagnostics_confirm_message")),
    ...
    private func cleanDiagnostics() {
        guard !isBusy else { return }
        isCleaningDiagnostics = true

        DispatchQueue.global(qos: .userInitiated).async {
            let result = DiagnosticCleanupService.clean()
            let message = language.text("cleaner.diagnostics_result", sizeText(result.freedBytes), Int64(result.removedItemCount), Int64(result.failedItemCount))
            let refreshed = DiagnosticCleanupService.scan()

            DispatchQueue.main.async {
                diagnosticUsage = refreshed
                isCleaningDiagnostics = false
...
private struct DiagnosticFilesView: View {
    let directory: DiagnosticDirectoryUsage
    let sizeText: (Int64) -> String
            ...
            List {
                ...
                if directory.files.isEmpty {
                    VStack(spacing: 8) {
                        ...
                        Text("No files")
                            .font(.headline)
                        Text("No regular files were found in this directory.")
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                            .multilineTextAlignment(.center)
                    }
...
private struct StorageTopFilesView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                Section {
                    Text("Menampilkan maksimal 100 file terbesar dari lokasi yang dipindai.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
...
private enum CleanerAlert: Identifiable {
    ...
    var id: String {
        switch self {
        case .diagnosticConfirmation: return "diagnostic-confirmation"
